...
export default function SuperAdmin({ user, onLogout }) {
  ...
  const [modal,      setModal]    = useState(null)   // 'brand'|'ad'|'banner'|'edit'|'editAd'|'editBanner'
    ...
    <div className="dash-layout fade" style={{position:'relative'}}>
      ...
      <div className="main-area">
        ...
        <div className="content-area">
                  ...
                  {brands.map(b => (
                    <div key={b.id} className="table-row" style={{gridTemplateColumns:'2.5fr 2fr 0.8fr 1fr 0.8fr auto'}}>
                      ...
                      <span style={{display:'flex',gap:6}}>
                        ...
                        <button className="btn sm" style={{background:b.gmb_connected?'rgba(61,214,140,.1)':'rgba(66,133,244,.1)',border:`1px solid ${b.gmb_connected?'rgba(61,214,140,.25)':'rgba(66,133,244,.25)'}`,color:b.gmb_connected?'var(--green)':'#6ab0ff'}}
                          onClick={async()=>{
                            try {
                              const r = await api.getGMBConnectURL(b.id)
                              window.open(r.oauth_url,'_blank')
                            } catch(e) { alert(e.message) }
                          }}>
                          {b.gmb_connected ? '✓ GMB' : '🔗 GMB'}
                        </button>
          ...
          {tab==='ads' && (
            <div className="fade">
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                <div style={{fontWeight:800,fontSize:15}}>Ad Campaigns <span style={{fontSize:12,fontWeight:400,color:'var(--muted)'}}>({ads.length}/3 max)</span></div>
              </div>
              <div className="table-wrap" style={{marginBottom:20}}>
                <div className="table-head" style={{gridTemplateColumns:'2fr 1.5fr 1fr 1fr auto'}}>
                  <span>Title</span><span>Brand</span><span>Views</span><span>Status</span><span>Actions</span>
                </div>
                {ads.map(a=>(
                  <div key={a.id} className="table-row" style={{gridTemplateColumns:'2fr 1.5fr 1fr 1fr auto'}}>
                    <span>
                      <div style={{fontWeight:600}}>{a.title}</div>
                      {a.image_url && <div style={{fontSize:10,color:'var(--green)'}}>● Has image</div>}
                    </span>
                    ...
                    <span style={{display:'flex',gap:4}}>
                      <button className="btn sm" onClick={()=>{setEditAd(a);setModal('editAd')}}>Edit</button>
              ...
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
                <div style={{fontWeight:800,fontSize:15}}>Promo Banners <span style={{fontSize:12,fontWeight:400,color:'var(--muted)'}}>Carousel — 3 seconds each</span></div>
              </div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:12}}>
                {banners.map(b=>(
                  <div key={b.id} className="card-box" style={{borderColor:b.active?'var(--goldb)':'var(--b1)',overflow:'hidden'}}>
                    {b.image_url && <img src={b.image_url} alt="" style={{width:'100%',height:70,objectFit:'cover',borderRadius:6,marginBottom:8}}/>}
                    {!b.image_url && <div style={{fontSize:26,marginBottom:6}}>{b.brand_emoji}</div>}
                    <div style={{fontWeight:700,fontSize:12,marginBottom:4}}>{b.title}</div>
                    <div style={{fontSize:11,color:'var(--muted)',marginBottom:8}}>{b.brand_name}</div>
                    <div style={{fontSize:11,color:b.active?'var(--green)':'var(--muted)',marginBottom:8}}>{b.active?'● Live':'○ Off'}</div>
                    <div style={{display:'flex',gap:4}}>
                      <button className="btn sm" style={{flex:1}} onClick={()=>{setEditBanner(b);setModal('editBanner')}}>Edit</button>
                      <button className={`btn sm ${b.active?'danger':'success'}`} style={{flex:1}} onClick={async()=>{await api.toggleBanner(b.id);load()}}>{b.active?'Hide':'Show'}</button>
                    </div>
                  </div>
                ))}
                {banners.length===0&&<div style={{color:'var(--muted)',fontSize:13,padding:20}}>No banners yet.</div>}
              </div>
            </div>
          )}
          ...
          {tab==='reviews' && (
            <div className="fade">
              <div style={{fontWeight:800,fontSize:15,marginBottom:4}}>All Google Reviews <span style={{fontSize:12,fontWeight:400,color:'var(--muted)'}}>({reviews.length})</span></div>
              <div style={{fontSize:12,color:'var(--muted)',marginBottom:14}}>Every review collected through ReviewRise QR codes — with customer email and coupon status.</div>

              {reviews.length===0 ? (
                <div style={{textAlign:'center',padding:60,color:'var(--muted)'}}>No reviews yet.</div>
              ) : reviews.map(r=>(
                <div key={r.id} style={{background:'var(--c2)',border:'1px solid var(--b1)',borderRadius:12,padding:14,marginBottom:8}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8,gap:10}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontWeight:700,fontSize:13}}>{r.reviewer_name||'Anonymous'}</div>
                      <div style={{fontSize:11,color:'var(--gold)',marginTop:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>📧 {r.user_email||'—'}</div>
                      <div style={{color:'#FBBC04',fontSize:13,marginTop:4}}>{'★'.repeat(r.stars||0)}{'☆'.repeat(5-(r.stars||0))}</div>
                    </div>
                    <div style={{textAlign:'right',flexShrink:0}}>
                      <div style={{fontSize:11,color:'var(--muted)'}}>{r.brand_emoji} {r.brand_name}</div>
                      <div style={{fontSize:11,color:'var(--muted)',marginTop:2}}>{new Date(r.created_at).toLocaleDateString('en-IN',{day:'2-digit',month:'short',year:'numeric'})}</div>
                      {r.coupon_code && (
                        <>
                          <div style={{fontFamily:'monospace',fontSize:11,fontWeight:700,color:'var(--gold)',marginTop:4}}>{r.coupon_code}</div>
                          <div style={{fontSize:10,color:r.coupon_status==='redeemed'?'var(--red)':r.coupon_status==='active'?'var(--green)':'var(--muted)'}}>
                            {r.coupon_status==='redeemed'?'🔴 Redeemed':r.coupon_status==='active'?'🟢 Active':'⚪ —'}
                          </div>
                        </>
                      )}
                    </div>
                  </div>
                  {r.review_text && <div style={{fontSize:12,color:'var(--muted2)',lineHeight:1.6,fontStyle:'italic',borderTop:'1px solid var(--b1)',paddingTop:8,marginTop:4}}>"{r.review_text}"</div>}
                </div>
              ))}
            </div>
          )}
          ...
          {tab==='feedback' && (
            <div className="fade">
              <div style={{fontWeight:800,fontSize:15,marginBottom:12}}>All Customer Feedback <span style={{fontSize:12,fontWeight:400,color:'var(--muted)'}}>({feedback.length})</span></div>
              {feedback.length===0 ? (
                <div style={{textAlign:'center',padding:60,color:'var(--muted)'}}>No feedback received yet.</div>
              ) : feedback.map(f=>(
                <div key={f.id} style={{background:'var(--c2)',border:'1px solid var(--b1)',borderRadius:12,padding:14,marginBottom:8}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:8}}>
                    <div>
                      <div style={{fontWeight:700,fontSize:13}}>{f.user_name}</div>
                      <div style={{fontSize:11,color:'var(--muted)'}}>{f.user_email}</div>
                    </div>
                    <div style={{textAlign:'right'}}>
                      <div style={{fontSize:11,color:'var(--muted)'}}>{f.brand_emoji} {f.brand_name}</div>
                      <div style={{fontSize:11,color:'var(--muted)',marginTop:2}}>{new Date(f.created_at).toLocaleDateString('en-IN')}</div>
                    </div>
                  </div>
                  <div style={{color:'#FBBC04',marginBottom:6}}>{'★'.repeat(f.stars)}{'☆'.repeat(5-f.stars)}</div>
                  {f.chips?.length>0 && (
                    <div style={{display:'flex',flexWrap:'wrap',gap:4,marginBottom:6}}>
                      {f.chips.map(c=><span key={c} style={{fontSize:11,background:'rgba(240,119,119,.1)',border:'1px solid rgba(240,119,119,.2)',borderRadius:100,padding:'2px 8px',color:'var(--red)'}}>{c}</span>)}
                    </div>
                  )}
                  {f.message && <div style={{fontSize:12,color:'var(--muted2)',lineHeight:1.6,fontStyle:'italic'}}>"{f.message}"</div>}
                </div>
              ))}
            </div>
          )}
      ...
      {modal==='ad' && (
        <Modal onClose={()=>setModal(null)}>
          ...
          <div style={{fontSize:12,color:'var(--muted)',marginBottom:14}}>Max 3 ads allowed</div>
          {ads.length >= 3 ? (
            <div style={{background:'rgba(240,119,119,.1)',border:'1px solid rgba(240,119,119,.2)',borderRadius:10,padding:14,fontSize:13,marginBottom:12,color:'var(--red)'}}>
              ⚠ Maximum 3 ads reached. Edit or pause an existing ad first.
            </div>
          ) : (
            <AdForm onClose={()=>setModal(null)} onAdd={async d=>{await api.createAd(d);load();setModal(null)}} brands={brands}/>
          )}
          {ads.length >= 3 && <button className="btn" style={{width:'100%',justifyContent:'center'}} onClick={()=>setModal(null)}>Close</button>}
        </Modal>
      )}

      {modal==='editAd' && editAd && (
        <Modal onClose={()=>{setModal(null);setEditAd(null)}}>
          <div style={{fontSize:16,fontWeight:900,marginBottom:14}}>Edit Ad Campaign</div>
          <AdForm
            initial={editAd}
            brands={brands}
            onClose={()=>{setModal(null);setEditAd(null)}}
            onAdd={async d=>{await api.updateAd(editAd.id,d);load();setModal(null);setEditAd(null)}}
      ...
      {modal==='banner' && (
        <Modal onClose={()=>setModal(null)}>
          <div style={{fontSize:16,fontWeight:900,marginBottom:4}}>Add Promo Banner</div>
          <div style={{fontSize:12,color:'var(--muted)',marginBottom:14}}>Appears in carousel on explore page (3 seconds each)</div>
          <BannerForm onClose={()=>setModal(null)} onAdd={async d=>{await api.createBanner(d);load();setModal(null)}} brands={brands}/>
        </Modal>
      )}

      {modal==='editBanner' && editBanner && (
        <Modal onClose={()=>{setModal(null);setEditBanner(null)}}>
          <div style={{fontSize:16,fontWeight:900,marginBottom:14}}>Edit Banner</div>
          <BannerForm
            initial={editBanner}
            brands={brands}
            onClose={()=>{setModal(null);setEditBanner(null)}}
            onAdd={async d=>{await api.updateBanner(editBanner.id,d);load();setModal(null);setEditBanner(null)}}
            submitLabel="Save Changes"
          />
        </Modal>
      )}
...
function ImageUploadField({ label, value, onChange, height=80 }) {
  const [uploading, setUploading] = useState(false)
  const [tab, setTab] = useState(value?.startsWith('http') || !value ? 'url' : 'url')
  ...
  const handleFile = async (file) => {
    ...
    setUploading(true)
    try {
      // Convert to base64 data URI — stored directly, no external service needed
  ...
  return (
    <div className="form-row">
      ...
      <div style={{display:'flex',gap:0,marginBottom:8,background:'var(--c2)',borderRadius:8,border:'1px solid var(--b1)',overflow:'hidden'}}>
        {['url','upload'].map(t=>(
          <button key={t} onClick={()=>setTab(t)}
            style={{flex:1,padding:'6px',background:tab===t?'rgba(233,184,74,.15)':'transparent',border:'none',color:tab===t?'var(--gold)':'var(--muted)',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit',textTransform:'uppercase',letterSpacing:'.05em',transition:'all .15s'}}>
            {t==='url'?'🔗 Paste URL':'📁 Upload File'}
          </button>
        ))}
      </div>
      {tab==='url' ? (
        <input className="input" placeholder="https://drive.google.com/... or any image URL"
        ...
        <div
          ...
          style={{border:'2px dashed rgba(233,184,74,.3)',borderRadius:10,padding:'20px',textAlign:'center',cursor:'pointer',background:'rgba(233,184,74,.03)',transition:'all .15s'}}
          onMouseEnter={e=>e.currentTarget.style.borderColor='rgba(233,184,74,.6)'}
          onMouseLeave={e=>e.currentTarget.style.borderColor='rgba(233,184,74,.3)'}>
          <div style={{fontSize:24,marginBottom:6}}>📸</div>
          <div style={{fontSize:12,color:'var(--muted2)',fontWeight:600}}>
            {uploading ? 'Processing...' : 'Click or drag & drop'}
          </div>
          <div style={{fontSize:10,color:'var(--muted)',marginTop:3}}>JPG, PNG, WebP · Max 5MB · Recommended: 1200×500px</div>
      ...
      {value && (
        <div style={{position:'relative',marginTop:8}}>
          <img src={value} alt="" style={{width:'100%',height,objectFit:'cover',borderRadius:8,display:'block'}}
            onError={e=>e.target.style.display='none'}/>
          <button onClick={()=>onChange('')}
            style={{position:'absolute',top:6,right:6,background:'rgba(0,0,0,.7)',border:'none',borderRadius:'50%',width:22,height:22,color:'white',cursor:'pointer',fontSize:12,display:'flex',alignItems:'center',justifyContent:'center'}}>
            ✕
          </button>
          {value.startsWith('data:') && <div style={{position:'absolute',bottom:6,left:6,background:'rgba(61,214,140,.9)',borderRadius:4,padding:'2px 6px',fontSize:10,fontWeight:700,color:'#000'}}>✓ Uploaded</div>}
        </div>
      )}
    </div>
  )
...
function AdForm({ onClose, onAdd, brands, initial, submitLabel='Create Ad' }) {
  const [f,setF] = useState({
  ...
  const [mediaTab,setMediaTab]= useState(initial?.video_url ? 'video' : 'image')
  ...
  return (
    <>
      <div className="form-row"><label className="label">Brand</label>
        <select className="input" value={f.brand_id} onChange={e=>setF(p=>({...p,brand_id:e.target.value}))}>
          {brands.map(b=><option key={b.id} value={b.id}>{b.emoji} {b.name}</option>)}
        </select>
      </div>
      <div className="form-row"><label className="label">Ad Title</label>
        <input className="input" placeholder="e.g. Summer Special — 30% OFF" value={f.title} onChange={e=>setF(p=>({...p,title:e.target.value}))}/>
      </div>
      <div className="form-row"><label className="label">Description</label>
        <textarea className="input" style={{height:60,resize:'none'}} placeholder="Short promo text shown under the ad..." value={f.description} onChange={e=>setF(p=>({...p,description:e.target.value}))}/>
      </div>

      {/* Media type tabs */}
      <div className="form-row">
        <label className="label">Media</label>
        <div style={{display:'flex',gap:0,marginBottom:10,background:'var(--c2)',borderRadius:8,border:'1px solid var(--b1)',overflow:'hidden'}}>
          {[{id:'image',lbl:'🖼 Image'},{id:'video',lbl:'▶ YouTube Video'}].map(t=>(
            <button key={t.id} onClick={()=>setMediaTab(t.id)}
              style={{flex:1,padding:'7px',background:mediaTab===t.id?'rgba(233,184,74,.15)':'transparent',border:'none',color:mediaTab===t.id?'var(--gold)':'var(--muted)',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit',transition:'all .15s'}}>
              {t.lbl}
            </button>
          ))}
        </div>
        {mediaTab==='image' ? (
          <ImageUploadField label="" value={f.image_url} onChange={v=>setF(p=>({...p,image_url:v}))} height={90}/>
        ) : (
          <>
            <input className="input" placeholder="https://youtube.com/watch?v=... or youtu.be/..."
              value={f.video_url} onChange={e=>setF(p=>({...p,video_url:e.target.value}))}/>
            <div style={{fontSize:10,color:'var(--muted)',marginTop:3}}>Paste any YouTube link — it will play inline in the ad</div>
            {ytId(f.video_url) && (
              <div style={{marginTop:8,borderRadius:8,overflow:'hidden',aspectRatio:'16/9'}}>
                <iframe width="100%" height="100%" style={{border:'none',display:'block'}}
                  src={`https://www.youtube.com/embed/${ytId(f.video_url)}?autoplay=0&rel=0`}
      ...
      <div style={{display:'flex',gap:8,marginTop:4}}>
        <button className="btn primary" style={{flex:1,justifyContent:'center'}} disabled={saving}
          onClick={async()=>{setSaving(true);try{await onAdd(f)}catch(e){alert(e.message)}setSaving(false)}}>
...
function BannerForm({ onClose, onAdd, brands, initial, submitLabel='Add Banner' }) {
  const [f,setF] = useState({
    brand_id:  initial?.brand_id  || brands[0]?.id || '',
    title:     initial?.title     || '',
  ...
  return (
    <>
      <div style={{background:'rgba(233,184,74,.06)',border:'1px solid rgba(233,184,74,.15)',borderRadius:10,padding:'10px 14px',fontSize:12,color:'rgba(237,233,226,.6)',marginBottom:14,lineHeight:1.7}}>
        📐 <strong style={{color:'var(--gold)'}}>Recommended banner size: 1200 × 500px</strong><br/>
        Ratio 2.4:1 · JPG/PNG/WebP · Max 5MB · Or paste a Google Drive / any image URL
      </div>
      <div className="form-row"><label className="label">Brand</label>
        <select className="input" value={f.brand_id} onChange={e=>setF(p=>({...p,brand_id:e.target.value}))}>
          {brands.map(b=><option key={b.id} value={b.id}>{b.emoji} {b.name}</option>)}
      ...
      <div className="form-row"><label className="label">Headline</label>
        <input className="input" placeholder="e.g. Get 30% OFF this weekend only!" value={f.title} onChange={e=>setF(p=>({...p,title:e.target.value}))}/>
      </div>
      <div className="form-row"><label className="label">Sub-text <span style={{color:'var(--muted)',fontWeight:400}}>(optional)</span></label>
        <input className="input" placeholder="e.g. Valid till Sunday · Min ₹500" value={f.subtitle} onChange={e=>setF(p=>({...p,subtitle:e.target.value}))}/>
      </div>
      <ImageUploadField label="Banner Image" value={f.image_url} onChange={v=>setF(p=>({...p,image_url:v}))} height={110}/>
      <div style={{display:'flex',gap:8,marginTop:4}}>
        <button className="btn primary" style={{flex:1,justifyContent:'center'}} disabled={saving}
          onClick={async()=>{setSaving(true);try{await onAdd(f)}catch(e){alert(e.message)}setSaving(false)}}>
          {saving?'Saving...':submitLabel}
